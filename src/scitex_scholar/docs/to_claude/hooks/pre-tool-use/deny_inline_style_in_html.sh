...
# File: ./src/.claude/to_claude/hooks/pre-tool-use/deny_inline_style_in_html.sh

# Description: Blocks inline styles in HTML files
...
if echo "$CONTENT" | grep -qiE '<style[^>]*>[^<]+</style>'; then
    ...
    echo "  - Move CSS to external .css files" >&2
    echo "  - Use <link rel=\"stylesheet\" href=\"path/to/file.css\">" >&2
    echo "" >&2
...
if echo "$CONTENT" | grep -qiE 'style\s*=\s*["\x27][^"\x27]+["\x27]'; then
    echo "BLOCKED: Inline style attributes detected in HTML file" >&2
    echo "" >&2
    echo "File: $FILE_PATH" >&2
    echo "" >&2
    echo "Best practice:" >&2
    echo "  - Use CSS classes instead of inline styles" >&2
    echo "  - Define styles in external .css files" >&2
    echo "" >&2
    echo "Why: Separation of concerns improves maintainability," >&2
