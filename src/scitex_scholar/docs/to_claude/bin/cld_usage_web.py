...
try:
    from scitex_browser.auth import GoogleAuthHelper
...
async def login_to_claude(
    page, email: str, password: str, timeout: int = 60000
) -> bool:
    ...
    try:
        ...
        if HAS_SCITEX_AUTH:
            ...
            if debug:
                print(
                    "[cld_usage_web] Using scitex.browser.auth.GoogleAuthHelper",
                    file=sys.stderr,
                )
            ...
            if debug:
                print(
                    "[cld_usage_web] scitex.browser.auth not available, using fallback",
                    file=sys.stderr,
                )
...
async def _fallback_google_login(
    page, email: str, password: str, timeout: int, debug: bool
) -> bool:
    ...
    try:
        ...
        try:
            ...
            if is_2fa:
                if debug:
                    print(
                        "[fallback] 2FA detected - waiting up to 60s for approval...",
                        file=sys.stderr,
                    )
                ...
                for _ in range(30):
                    ...
                    try:
                        ...
                        if debug:
                            print(
                                "[fallback] 2FA completed - popup closed",
                                file=sys.stderr,
                            )
                        break
        except:
        ...
        try:
            ...
            if debug:
                ...
                try:
                    await popup.screenshot(path="/tmp/google_popup_debug.png")
                    print(
                        "[fallback] Popup screenshot: /tmp/google_popup_debug.png",
                        file=sys.stderr,
                    )
                except:
                    pass
...
async def get_usage_data(
    account_num: int = 1, headless: bool = True, timeout: int = 60000
) -> dict:
    """
    ...
    if not email or not password:
        usage_data["error"] = (
            f"Credentials not found for account {account_num}. Set ANTHROPIC_{account_num}_EMAIL and ANTHROPIC_{account_num}_PW"
        )
        return usage_data
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                needs_login = (
                    "login" in current_url or "Continue with Google" in page_content
                )
            except:
            ...
            if debug:
                ...
                print(
                    "[cld_usage_web] Screenshot: /tmp/claude_usage_debug.png",
                    file=sys.stderr,
                )
            ...
            if (
                len(percent_matches) >= 3
                and usage_data["weekly_sonnet_percent"] is None
            ):
                usage_data["weekly_sonnet_percent"] = int(percent_matches[2])

            # Strategy 3: Look for specific sections
            plan_match = re.search(
                r"Plan usage.*?(\d+)\s*%", body_text, re.IGNORECASE | re.DOTALL
            )
            ...
            all_models_match = re.search(
                r"All models.*?(\d+)\s*%", body_text, re.IGNORECASE | re.DOTALL
            )
            ...
            sonnet_match = re.search(
                r"Sonnet.*?(\d+)\s*%", body_text, re.IGNORECASE | re.DOTALL
            )
...
def main():
    ...
    parser.add_argument(
        "--account", "-a", type=int, default=1, help="Account number (1, 2, ...)"
    )
